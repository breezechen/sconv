...
std::wstring gbk_to_unicode(const std::string gbk_str)
{
    int size = sconv_gbk_to_unicode(gbk_str.c_str(), gbk_str.size(), 0, 0);
    wchar* unicode = new wchar[size / 2 + 1];
    size = sconv_gbk_to_unicode(gbk_str.c_str(), gbk_str.size(), unicode, size / 2 + 1);
    unicode[size / 2] = 0;
...
std::string unicode_to_gbk(const std::wstring& unicode_str)
{
...
std::wstring utf8_to_unicode(const std::string& utf8_str)
{
    ...
    wchar* unicode = new wchar[size / 2 + 1];
    size = sconv_utf8_to_unicode(utf8_str.c_str(), utf8_str.size(), unicode, size / 2 + 1);
    unicode[size / 2] = 0;
    std::wstring ret((wchar_t*)unicode, size / 2);
    delete[] unicode;
    return ret;
...
std::string unicode_to_utf8(const std::wstring& unicode_str)
{
